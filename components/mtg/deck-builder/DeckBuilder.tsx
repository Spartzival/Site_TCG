...
import DeckRecommendationsPanel from "./DeckRecommendationsPanel";
...
export default function DeckBuilder({
  ...
  const [recommendations, setRecommendations] = useState<DeckRecommendations | null>(null);
  ...
  const [analysisDrawer, setAnalysisDrawer] = useState<AnalysisDrawer | null>(null);
  ...
  useEffect(() => {
    if (!analysisDrawer) return;

    const previousOverflow = document.body.style.overflow;
    document.body.style.overflow = "hidden";

    const onKeyDown = (event: KeyboardEvent) => {
      if (event.key === "Escape") setAnalysisDrawer(null);
    };
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(async () => {
      ...
      try {
        const payload = await fetchJson<DeckRecommendations>(
          "/api/mtg/decks/recommendations",
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            signal: controller.signal,
        ...
        if (!controller.signal.aborted) {
          setRecommendations({ available: false, suggestions: [] });
        }
      } finally {
  ...
  const addComboCardByName = async (name: string) => {
    try {
      ...
      window.alert(`Impossible de récupérer « ${name} » depuis Scryfall.`);
    }
  };
  ...
  return (
    <div className={`mtg-deck-builder ${readOnly ? "is-readonly" : ""}`}>
      ...
      <div className="mtg-deck-builder__workspace">
        ...
        <aside className="mtg-deck-analysis-dock" aria-label="Analyse du deck">
          <div className="mtg-deck-analysis-dock__heading">
            <span>ANALYSE DU DECK</span>
            <strong>Ouvre un panneau →</strong>
          </div>

          <button
            type="button"
            className="mtg-analysis-launch-card"
            onClick={() => {
              setSelectedEntryKey(null);
              setAnalysisDrawer("stats");
            }}
          >
            <span className="mtg-analysis-launch-card__symbol">Σ</span>
            <span className="mtg-analysis-launch-card__body">
              <small>STRUCTURE</small>
              <strong>Stats & courbe</strong>
              <em>{local.averageManaValue.toFixed(2)} MV · {local.landCount} terrains</em>
            </span>
          ...
          <button
            type="button"
            className="mtg-analysis-launch-card is-recommendation"
            onClick={() => {
              setSelectedEntryKey(null);
              setAnalysisDrawer("recommendations");
            }}
          >
            <span className="mtg-analysis-launch-card__symbol">✦</span>
            <span className="mtg-analysis-launch-card__body">
              <small>CONSTRUCTION</small>
              <strong>Cartes clés</strong>
              <em>
                {recommendationsLoading
                  ? "Analyse…"
                  : `${recommendations?.suggestions.length ?? 0} suggestion(s)`}
              </em>
            </span>
            <span className="mtg-analysis-launch-card__arrow">↗</span>
          </button>
      ...
      {analysisDrawer && (
        <div
          className="mtg-analysis-drawer-backdrop"
          role="presentation"
          onMouseDown={(event) => {
            if (event.target === event.currentTarget) setAnalysisDrawer(null);
          }}
        >
          <aside
            className="mtg-analysis-drawer"
            role="dialog"
            aria-modal="true"
            aria-label="Analyse détaillée du deck"
          >
            <header className="mtg-analysis-drawer__header">
              <div>
                <span>DECK INTELLIGENCE</span>
                <strong>
                  {analysisDrawer === "stats"
              ...
              <button
                type="button"
                className="mtg-analysis-drawer__close"
                onClick={() => setAnalysisDrawer(null)}
            ...
            <div className="mtg-analysis-drawer__content">
              ...
              {analysisDrawer === "combos" && (
                <DeckComboPanel
                  ...
                  loading={remoteLoading}
                  readOnly={readOnly}
                  onAddMissingCard={readOnly ? undefined : addComboCardByName}
                />
              )}

              {analysisDrawer === "recommendations" && (
